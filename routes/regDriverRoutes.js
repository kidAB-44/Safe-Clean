...
// Create an object for storing photos or files to disk, using the multer middleware.
var storage = multer.diskStorage({
    destination: (req, file, cb) => {
        // Destination folder for uploaded photos or files
...
/* Define the endpoint from which data, photos or files for newly registered drivers will be uploaded using the DriverReg model.
** The endpoint here is /driver, from the file "registerDriver.pug".
*/
router.post('/', upload.single('fileUpload'), async (req, res) => {
    try {
        const newDriver = new DriverReg(req.body);
        // Save the path for uploaded photos or file.
        newDriver.fileUpload = req.file.path;
        await newDriver.save();
        // Render the data table for drivers.
